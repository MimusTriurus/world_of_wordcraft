...
// таблица целиком укладывается в холст при десяти строках
{
  const BEST_HEAD = 30, BEST_STEP = 26;
  const h = 62 + BEST_HEAD + 10 * BEST_STEP + 26;
  ok(`при десяти рекордах экран ${h} px из ${g.H}`, h <= g.H - 12);
}

// ---- 7. демо не показывает таблицу ------------------------------------------
g.startAttract();
g.state = "over";
g.draw();
